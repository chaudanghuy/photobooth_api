...
$(document).ready(function () {
  // Giữ selected khi reload
  const urlParams = new URLSearchParams(window.location.search);
  const selectedDevice = urlParams.get("device");
  if (selectedDevice) {
    $("#device_id").val(selectedDevice);
  }
  // Load frames
  loadFrames(selectedDevice);
});
...
$("body").on("click", ".edit-frame", function (e) {
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      editFrameForm
        .find("#photo_hover_preview")
        .attr("src", response.photo_hover);
...
function loadFrames(deviceId) {
  $(".api-loading").removeClass("d-none");
  $(".add-frame-container").addClass("d-none");
  $(".image-list").empty();

  let apiUrl = "/frames/api";
  if (deviceId) {
      ...
      frames.forEach((frame) => {
        $(".image-list").append(
          '<div class="image-container">' +
            '<img src="' +
            frame.photo +
            '" alt="frame" class="image-frame" data-frame-id="' +
            frame.id +
            '">' +
            ...
            '<a class="edit-frame text-decoration-none text-white" data-frame-id="' +
            frame.id +
            '">' +
            '<span class="mdi mdi-pencil"></span> Edit</a>' +
            "</button>" +
            '<button class="delete-button">' +
            '<a class="delete-frame text-decoration-none text-white" data-frame-id="' +
            frame.id +
            '">' +
